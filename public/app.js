...
(function () {
  ...
  function renderAiContent() {
    ...
    if (contentEl) {
      contentEl.innerHTML = marked.parse(currentAiText);
    }
  }
  ...
  function handleToolUse(msg) {
    if (!currentAiMsg) return;
    const toolId = 'tool-' + (msg.tool || 'default') + '-' + Date.now();
    ...
    if (msg.status === 'running') {
      ...
      // vor msg-content einfügen
      const contentEl = currentAiMsg.querySelector('.msg-content');
      currentAiMsg.insertBefore(indicator, contentEl);
    } else if (msg.status === 'done') {
      // letzten laufenden indikator für dieses tool finden
      ...
      if (indicator) {
        ...
        const doneLabel = label.replace('Durchsuche', 'Dokumentation durchsucht')
          .replace('Lese', 'Dokument gelesen')
          .replace('Lade', 'Übersicht geladen')
          .replace('Prüfe', 'Status geprüft');
        // fallback: einfach label mit erledigt-suffix
